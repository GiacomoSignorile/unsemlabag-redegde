...
class Hough:
    def __init__(self, pixel_res=1, angle_res=180, min_theta= -0.05, max_theta= 0.05, min_voting=160):
    ...
    def forward(self, image, rho_old=[], theta_old=-1, x_old=[], horizontal_exceed={}):
        ...
        for line in lines:
            ...
            cv2.line(combined_mask, (x1, 0), (x2, image.shape[1]), (255, 255, 255), 15)

        # Propagate and process combined mask
        combined_mask = self.propagate_horizontal_rows(combined_mask, x1, x_old, rho_old, theta_old)
        final_mask, horizontal_dict = self.generate_hough_line_label(combined_mask)
        return final_mask, horizontal_dict, rho, theta, x2, combined_mask
    ...
    def generate_hough_line_label(self, mask):  # , real_mask):
        ...
        print("Generating Hough line label")

        print("Binary mask shape:", self.binary_mask.shape)
